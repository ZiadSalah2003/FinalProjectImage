...
function gray_img = ConvertToGrayDesaturation(img)
    ...
    %(max(R(i, j),G(i, j),B(i, j)) + min(R(i, j),G(i, j),B(i, j))/2;
    for i = 1:M
        for j = 1:N
            gray_img(i,j) = (max([R(i, j),G(i, j),B(i, j)]) + min([R(i, j),G(i, j),B(i, j)]))/2;
        end
    end
